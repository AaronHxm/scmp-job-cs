...
public class ContractService {
    ...
    public QueryResponse<ContractInfo> fetchAllPreGrabCases(User user) {
        // 先获取第一页数据以确定总页数
        QueryResponse<ContractInfo> firstPageResponse = fetchPreGrabCases(1,user);
        ...
        List<CompletableFuture<QueryResponse<ContractInfo>>> futures = IntStream.range(2, totalPages + 1)
                .mapToObj(page -> CompletableFuture.supplyAsync(() -> fetchPreGrabCases(page,user), processingExecutor))
    ...
    public QueryResponse<ContractInfo> fetchPreGrabCases(int pageIndex,User user) {


        return fetchPreGrabCases(pageIndex,user.getToken());
